...
```{r}
sanitation_ETH_2000_2020_san_sm <- sanitation |> 
  filter(iso3 == "ETH",
         year == 2000 | year == 2020,
...
```{r}
sanitation_ETH_2000_2020_san_sm |> 
  count(year)
```
...
```{r}
sanitation_ETH_2000_2020_san_sm |> 
  ggplot(sanitation_ETH_2000_2020_san_sm,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The legend matches the order of the data in the bars from top to bottom regardless of the actual data values

2.  What would you want to change?

    One could not want the order to always be the same if one were to look at one part individually, but as a whole, it seems OK to me currently

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    Because it seems to be a total for categories considered safely managed, leaving it in would cause not very visually appealing or readable diagrams with bars possibly extending over 100%
...
2.  What do you observe when you look at the code and plot?
    ...
    The y axis automatically became count, without direct assignment and the legend is positioned differently to how it was in task 5, presumably due to lengend.position = "none"
